...
public class HomePageServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            request.setAttribute("CARTICON", Utils.getimage("cart"));
            
            ItemStatusDAO stDao = new ItemStatusDAO();
            String StatusID = stDao.getStatusID("Active");
            
            ItemCategoryDAO dao = new ItemCategoryDAO();
            List<ItemCategoryDTO> dto = dao.getCategoryList(StatusID);
            request.setAttribute("CATEGORY", dto);
            
            ItemStorageDAO stoDao = new ItemStorageDAO();
            List<ItemStorageDTO> recList_1 = stoDao.getRecommended_byTopSeller(StatusID);
            List<ItemStorageDTO> recList_2 = stoDao.getRecommended_byPopular(StatusID);
            
            request.setAttribute("TOPSELLER", recList_1);
            request.setAttribute("POPULAR", recList_2);
